 ...
 *   In this case Text\Util\ReflectionTypeHint::isValid() always returns TRUE!
 *
 * Useful links
...
class ReflectionTypeHint
{
    ...
    public static function isValid()
    {
        ...
        if (count($parameters) > count($params)) {
            ...
            $message = sprintf($message, count($parameters), $class, $type, $function, count($params), $file, $line,
        ...
        foreach ($args as $i => $value) {
            ...
            if ($parameters[$i]->name !== $params[$i][2]) {
                ...
                $message = 'phpDoc @param %d in %s%s%s() must be named as $%s, $%s given, ' . PHP_EOL
                    . 'called in %s on line %d ' . PHP_EOL
                    . 'and defined in %s on line %d';
                $message = sprintf($message, $param_num, $class, $type, $function, $parameters[$i]->name,
                    $params[$i][2], $file, $line, $r->getFileName(), $r->getStartLine());
                trigger_error($message, E_USER_NOTICE);
            }
            ...
            if (!self::checkValueTypes($hints, $value)) {
                ...
                $message = 'Argument %d passed to %s%s%s() must be an %s, %s given, ' . PHP_EOL
                    . 'called in %s on line %d ' . PHP_EOL
                    . 'and defined in %s on line %d';
                $message = sprintf($message, $param_num, $class, $type, $function, implode('|', $hints),
                    (is_object($value) ? get_class($value) . ' ' : '') . gettype($value), $file, $line,
                    $r->getFileName(), $r->getStartLine());
