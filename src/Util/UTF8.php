 ...
 * Example:
 *   $s = 'Hello, Привет';
 *   if (Text\Util\UTF8::is_utf8($s)) echo Text\Util\UTF8::strlen($s);
 *
...
class UTF8
{
    ...
    private static function _convert($data, $charset_from, $charset_to)
    {
        ...
        if (is_string($data)) {
            ...
            trigger_error('Convert "' . $charset_from . '" --> "' . $charset_to
                . '" is not supported native, "iconv" or "mbstring" extension required', E_USER_WARNING);
            return false;
        }
        return $data;
    }
     ...
     *   Text\Util\UTF8::blocks_check('поисковые системы Google и Yandex', array('Basic Latin', 'Cyrillic'));
     *   #You can check the named, direct ranges or codepoints together:
     *   Text\Util\UTF8::blocks_check('поисковые системы Google и Yandex', array(array(0x20, 0x7E),     #[\x20-\x7E]
    ...
    public static function autoconvert_request($is_hex2bin = false, $charset = 'cp1251')
    {
        ...
        foreach (['_GET', '_POST', '_COOKIE', '_FILES'] as $k => $v) {
            ...
            $GLOBALS[$v] = self::_autoconvert_request_recursive($GLOBALS[$v], $is_converted, $is_broken, $is_hex2bin,
                $charset);
            if ($is_broken) {
    ...
    public static function html_entity_decode($s, $is_special_chars = false)
    {
        ...
        #speed improve
        if (
            strlen($s) < 4  #по минимальной длине сущности - 4 байта: &#d; &xx;
            || ($pos = strpos($s, '&') === false)
            || strpos($s, ';', $pos) === false
        ) {
            return $s;
    ...
    public static function unescape_request()
    {
        ...
        $HTTP_RAW_POST_DATA = isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST'
        ...
        foreach ([
            '_GET' => isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : null,
    ...
    public static function str_pad($s, $length, $pad_str = ' ', $type = STR_PAD_RIGHT)
    {
        ...
        trigger_error('Parameter 4 should be a constant of STR_PAD_RIGHT, STR_PAD_LEFT or STR_PAD_BOTH!',
            E_USER_WARNING);
        return false;
    }
        ...
        while (($name = readdir($dh)) !== false) {
            ...
            if (is_file($file)) {
                ...
                if (!is_string($dirs_ignore_re) || !preg_match($dirs_ignore_re, $name)) {
                    $c = self::convert_files_from($file, $files_re, $is_recursive, $charset, $dirs_ignore_re, $is_echo,
                        $is_simulate);
                    if ($c === false) {
                        return false;
                    }
    ...
    public static function range($low, $high, $step = 1)
    {
        ...
        return array_map(['Text\Util\UTF8', 'chr'], $a);
    }
